...
const EditLeaveGroup = () => {
        ...
        const fetchLeaveGroup = async () => {
            try {
                ...
                if (data.success) {
                    ...
                    setFormData({
                        ...lg,
                        leaveIntimationEmails: Array.isArray(lg.leaveIntimationEmails)
                            ? lg.leaveIntimationEmails.join(', ')
                            : lg.leaveIntimationEmails || '',
    ...
    if (loading) return (
        <div className="hrm-container" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '60vh' }}>
            <div style={{ textAlign: 'center', color: '#64748B', fontSize: '15px', fontWeight: 600 }}>Loading...</div>
        </div>
    );
